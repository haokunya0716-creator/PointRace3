...
#define GPIO_FAST_SETPIN(port, pin)  DL_GPIO_setPins(port, pin)
#define GPIO_FAST_RSTPIN(port, pin)  DL_GPIO_clearPins(port, pin)
...
static void st7735s_write(uint8_t data)
{
  ...
  DL_SPI_transmitData8(ST7735S_SPI_INST, data);
  while(DL_SPI_isBusy(ST7735S_SPI_INST));
#else
  // 软件SPI模拟
...
void st7735s_init(void)
{
  ...
  GPIO_FAST_SETPIN(ST7735S_LCD_BL_GPIO_Port, ST7735S_LCD_BL_Pin);
#endif
